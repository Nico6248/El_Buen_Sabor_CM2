...
def create_dish(dish: DishCreate, session: Session = Depends(get_session), current_user=Depends(admin_required)):
...
def get_menu_for_restaurant(restaurant_id: int, session: Session = Depends(get_session), current_user=Depends(get_current_user)):
